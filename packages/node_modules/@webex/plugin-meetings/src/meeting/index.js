...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
  ...
  async updateShare(options) {
    ...
    try {
      // const p1 =
      await MeetingUtil.validateOptions({sendShare, localShare: stream});

      // console.log('step1', p1);
      const startShare = await this.checkForStopShare(sendShare, previousSendShareStatus);

      // console.log('step2', startShare);
      // const p3 =
      await MeetingUtil.updateTransceiver({
        type: 'video',
        sendTrack: sendShare,
      ...
      // console.log('step4', this.mediaProperties.mediaDirection);
    }
    catch (e) {
      // console.warn(e);
    }
  ...
  setMeetingQuality(level) {
    ...
    this.mediaProperties.setQualityLevel(level);
    ...
    // If we are receiving media, set the remote media quality
    if (receiveAudio || receiveVideo) {
      qualityActions.push(this.setRemoteQualityLevel(level));
    }

    // If we are sending media, set the local media quality
    if (sendVideo) {
      qualityActions.push(this.setLocalVideoQuality(level));
    }

    return Promise.all(qualityActions);
  }
}
